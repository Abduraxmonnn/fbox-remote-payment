...
export default function PaymentServices() {
  ...
  const fetchTransactionData = async (service) => {
    setSelectedService(service)

    const body = {
      params: {
        source: service,
        first: localStorage.getItem('transactionId'),
      },
    }
    ...
    try {
      ...
      const data = response.data.results.map((data) => ({
        ...
        marketLogo: data.logo,
        marketBanner: data.banner
      }));
