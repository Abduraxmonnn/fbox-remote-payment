...
export default function PaymentMethodGrid({ paymentMethods, selectedTip, customTipAmount, theme, handleButtonClick }) {
        ...
        <div className="payment-methods-grid">
            {paymentMethods.map((method) => {
              ...
              ${isDisabled ? "coming-soon-card" : ""}`}
                        onClick={!isDisabled ? () => handleButtonClick(method) : undefined}
                    >
                        ...
                        <div className="method-icon">
                            <img
                                src={isDisabled ? method.inactive_icon || "/placeholder.svg" : method.icon || "/placeholder.svg"}
                                alt={`${method.name} icon`}
                                className="icon-image"
                            />
                        </div>

                        <span className={`payment-provider ${theme === "dark" && method.isPopular ? "popular-provider" : ""}`}>
              {method.name}
            </span>
                    </div>
                )
